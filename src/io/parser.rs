...
use crate::io::json_format::{
    JsonCP, JsonCPNode, JsonCPNodeType, JsonInstance, JsonOrientation, JsonSolution,
    JsonSolutionStats,
};
...
pub fn convert_node_bp_to_json_cp_node(node: &NodeBlueprint) -> JsonCPNode {
    ...
    for child in node.children.iter().sorted_by(|a, b| {
        a.calculate_usage()
            .partial_cmp(&b.calculate_usage())
            .unwrap()
            .reverse()
    }) {
