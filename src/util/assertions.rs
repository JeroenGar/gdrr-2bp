...
pub fn children_nodes_fit(node_i: &NodeKey, arena: &SlotMap<NodeKey, Node>) -> bool {
    ...
    match node.has_children() {
        false => true,
        true => match node.next_cut_orient() {
            Orientation::Horizontal => {
                let all_children_same_width = node
                    .children(arena)
                    .all(|c| arena[c].width() == node.width());
                let sum_of_children_height =
                    node.children(arena).map(|c| arena[c].height()).sum::<u64>();
                let all_children_vert_cut_orient = node
                    .children(arena)
                    .all(|c| arena[c].next_cut_orient() == Orientation::Vertical);

                if !all_children_same_width
                    || sum_of_children_height != node.height()
                    || !all_children_vert_cut_orient
                {
                    return false;
                }
            ...
            Orientation::Vertical => {
                let all_children_same_height = node
                    .children(arena)
                    .all(|c| arena[c].height() == node.height());
                let sum_of_children_width =
                    node.children(arena).map(|c| arena[c].width()).sum::<u64>();
                let all_children_horz_cut_orient = node
                    .children(arena)
                    .all(|c| arena[c].next_cut_orient() == Orientation::Horizontal);

                if !all_children_same_height
                    || sum_of_children_width != node.width()
                    || !all_children_horz_cut_orient
                {
                    return false;
                }
                node.children(arena).all(|c| children_nodes_fit(&c, arena))
...
pub fn children_node_blueprints_fit(node_bp: &NodeBlueprint) -> bool {
    match node_bp.children.is_empty() {
        true => true,
        false => match node_bp.next_cut_orient {
            Orientation::Horizontal => {
                let all_children_same_width =
                    node_bp.children.iter().all(|nb| nb.width == node_bp.width);
                let sum_of_children_height =
                    node_bp.children.iter().map(|nb| nb.height).sum::<u64>();
                let all_children_vert_cut_orient = node_bp
                    .children
                    .iter()
                    .all(|nb| nb.next_cut_orient == Orientation::Vertical);

                if !all_children_same_width
                    || sum_of_children_height != node_bp.height
                    || !all_children_vert_cut_orient
                {
                    return false;
                }
                node_bp
                    .children
                    .iter()
                    .all(|nb| children_node_blueprints_fit(nb))
            }
            Orientation::Vertical => {
                let all_children_same_height = node_bp
                    .children
                    .iter()
                    .all(|nb| nb.height == node_bp.height);
                let sum_of_children_width = node_bp.children.iter().map(|nb| nb.width).sum::<u64>();
                let all_children_horz_cut_orient = node_bp
                    .children
                    .iter()
                    .all(|nb| nb.next_cut_orient == Orientation::Horizontal);

                if !all_children_same_height
                    || sum_of_children_width != node_bp.width
                    || !all_children_horz_cut_orient
                {
                    return false;
                }
...
pub fn nodes_match(
    n_i_1: &NodeKey,
    n_i_2: &NodeKey,
    nodes_1: &SlotMap<NodeKey, Node>,
    nodes_2: &SlotMap<NodeKey, Node>,
) -> bool {
...
pub fn insertion_option_cache_is_valid<'a>(
    ...
    fresh_ioc.add_for_parttypes(parttypes, layouts_to_consider());
    ...
    for (layout_index, layout) in layouts_to_consider() {
        for node_index in layout.sorted_empty_nodes() {
            ...
            let ioc_options = ioc
                .options_for_node(node_index, &layout_index)
                .filter(|option| parttypes.contains(&option.parttype()))
                .collect_vec();
            let fresh_ioc_options = fresh_ioc
                .options_for_node(node_index, &layout_index)
                .collect_vec();
...
pub fn cached_sorted_empty_nodes_correct(
    nodes: &SlotMap<NodeKey, Node>,
    cached_sorted_empty_nodes: &[NodeKey],
) -> bool {
    let all_empty_nodes = nodes
        .iter()
    ...
    if !all_empty_nodes
        .iter()
        .all(|n| cached_sorted_empty_nodes.contains(n))
    {
        return false;
    }
...
pub fn cached_used_part_area_correct(
    nodes: &SlotMap<NodeKey, Node>,
    cached_used_part_area: u64,
) -> bool {
    nodes
        .iter()
        ...
        .map(PartType::area)
        .sum::<u64>()
        == cached_used_part_area
}

pub fn cached_removable_nodes_correct(
    nodes: &SlotMap<NodeKey, Node>,
    removable_nodes: &[NodeKey],
) -> bool {
...
pub fn instance_parttypes_and_sheettypes_ids_correct(
    parttypes: &[(PartType, usize)],
    sheettypes: &[(SheetType, usize)],
) -> bool {
    parttypes
        .iter()
        .enumerate()
...
pub fn no_ghost_nodes_in_arena(nodes: &SlotMap<NodeKey, Node>, top_node: &NodeKey) -> bool {
    ...
    nodes.iter().all(|(i, _n)| referenced_indices.contains(&i))
}

pub fn node_child_parent_relations_valid(
    nodes: &SlotMap<NodeKey, Node>,
    top_node: &NodeKey,
) -> bool {
    ...
    }) && nodes.iter().all(|(node_key, node)| match node.parent() {
        Some(parent_key) => nodes
            .get(parent_key)
