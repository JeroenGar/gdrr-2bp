...
- `6a1757f` - use `Option::unwrap_or_else` when creating a solution so an already supplied cost no longer triggers a discarded full-layout cost scan. Sequential immediate-parent Criterion improved throughput by 2.84% (`+1.53%..+4.41%`, `p = 0.00`); exact behavior. The 60-second checkpoint and PR overview row are deferred to final readability verification.
- `91466ee` - encapsulate live, detached, sampled, and changed-layout membership in a private `ProblemLayouts` type. Full-solver Criterion found no measurable change at -0.94% versus `cf083c2` (`-3.29%..+1.45%`, `p = 0.46`); exact normalized seeded 50,000-iteration behavior; no public API or runtime representation change. The final 60-second branch checkpoint reached 86,040 iter/s, with a complete 291-sheet solution at 98.464% usage and an incomplete 290-sheet state at 98.626% usage/99.820% included.
- `2ad1969` - encapsulate the node arena, topology, used-area cache, and node indexes in a private `LayoutNodes` type. Full-solver Criterion found no measurable change at +0.36% versus `e87788d` (`-1.34%..+2.00%`, `p = 0.69`); exact normalized seeded 50,000-iteration behavior; no public API or runtime representation change.
