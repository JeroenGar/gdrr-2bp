...
- `91466ee` - encapsulate live, detached, sampled, and changed-layout membership in a private `ProblemLayouts` type. Full-solver Criterion found no measurable change at -0.94% versus `cf083c2` (`-3.29%..+1.45%`, `p = 0.46`); exact normalized seeded 50,000-iteration behavior; no public API or runtime representation change.
